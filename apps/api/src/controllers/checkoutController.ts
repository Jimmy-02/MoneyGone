...
export async function createCheckout(req: Request, res: Response, next: NextFunction) {
    try{
        ...
        if (!parsed.success) {
            res.status(400).json({ error: "Invalid cart format", details: parsed.error.flatten() });
            return;
        }

        if(!env.POLAR_ACCESS_TOKEN) {
            res.status(503).json({ error: "Payment configuration error" });
            return;
        }
        ...
        const checkout = await polarCreateCheckout(env, {
            ...
            success_url: successUrl.replace("{CHECKOUT_ID}", session.id),
