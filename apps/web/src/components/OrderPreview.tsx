...
import { IK_PRESETS } from "../lib/imageKitUrl";
import { imageKitOptimizedUrl } from "../lib/imageKitUrl";
import { OrderPreviewItem, OrderPreviewProps, SIZES } from "../types/order";
...
export function OrderPreview({ items, size = "md" }: OrderPreviewProps) {
  ...
  return (
    <div className={`grid shrink-0 grid-cols-2 grid-rows-2 gap-0.5 overflow-hidden rounded-2xl border border-base-300 bg-base-200/90 p-0.5 shadow-md ring-1 ring-base-300/40 ${box}`}>
      {show.map((p: OrderPreviewItem, i: number) => (
        <div key={`${p.slug}-${i}`} className="relative min-h-0 overflow-hidden rounded-md bg-base-300">
          {p.imageUrl ? (
            <img
              src={imageKitOptimizedUrl(p.imageUrl, ikPreset)}
              alt=""
