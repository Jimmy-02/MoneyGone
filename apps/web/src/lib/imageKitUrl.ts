 ...
 */
function buildMoneygoneTextLayer({ w, h }: Pick<TrOpts, "w" | "h">): string {
  ...
  return `l-text,i-Moneygone,fs-${fs},co-FFFFFF,bg-0F172A90,pa-8_12,lx-N14,ly-14,lap-top_right,l-end`;
}
...
function buildTrSegment({
  ...
  return `${base}:${buildMoneygoneTextLayer({ w, h })}`;
}
...
/**
 * Same optimizations as {@link imageKitOptimizedUrl} plus Moneygone text overlay (for share/download).
...
export const IK_PRESETS: Record<string, TrOpts> = {
